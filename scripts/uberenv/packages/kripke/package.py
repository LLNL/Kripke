...
class Kripke(CMakePackage, CudaPackage):
    ...
    variant('raja', default=True, description='Build with RAJA, CHAI, and Umpire')
    ...
    depends_on('chai+raja', when='+raja')
    depends_on('chai+raja+cuda', when='+raja+cuda')
...
#            -DHIP_ROOT_DIR=/opt/rocm-3.6.0/hip -DHIP_CLANG_PATH=/opt/rocm-3.6.0/llvm/bin
        ...
        if "+raja" in spec:
            ...
            cfg.write(cmake_cache_option("ENABLE_RAJA", True))

            cfg.write(cmake_cache_entry("RAJA_DIR", "{0}/share/raja/cmake".format(spec['raja'].prefix)))
            cfg.write(cmake_cache_entry("chai_DIR", "{0}/share/chai/cmake".format(spec['chai'].prefix)))
            cfg.write(cmake_cache_entry("umpire_DIR", "{0}/share/umpire/cmake".format(spec['umpire'].prefix)))
        else:
            cfg.write(cmake_cache_option("ENABLE_CHAI", False))
            cfg.write(cmake_cache_option("ENABLE_RAJA", False))
