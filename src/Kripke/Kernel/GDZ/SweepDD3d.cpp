...
void gdz_nmd_SweepDD_3d(gdz_nmd_Param &p) {
  ...
  gdz_TVec psi_lf(num_groups, num_directions,
                  (local_imax+1)*local_jmax*local_kmax);
  gdz_TVec psi_fr(num_groups, num_directions,
                  local_imax*(local_jmax+1)*local_kmax);
  gdz_TVec psi_bo(num_groups, num_directions,
                  local_imax*local_jmax*(local_kmax+1));

  gdz_TVec i_plane_v(num_groups, num_directions, local_jmax*local_kmax);
  gdz_TVec j_plane_v(num_groups, num_directions, local_imax*local_kmax);
  gdz_TVec k_plane_v(num_groups, num_directions, local_imax*local_jmax);
  double ***i_plane = i_plane_v.data;
  double ***j_plane = j_plane_v.data;
  double ***k_plane = k_plane_v.data;

  gdz_TVec psi_internal(num_groups, num_directions, num_zones);
