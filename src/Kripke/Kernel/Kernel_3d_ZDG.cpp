...
void Kernel_3d_ZDG::scattering(Grid_Data *grid_data){
  ...
  for(int zs = 0;zs < grid_data->num_zone_sets;++ zs){
    ...
    View3d<double, LAYOUT_KIJ> phi_out(sdom.phi_out->ptr(), num_moments, num_groups, num_zones);
    View3d<double, LAYOUT_KIJ> const phi(sdom.phi->ptr(), num_moments, num_groups, num_zones);  
    View4d<double, LAYOUT_LIJK> const sigs(grid_data->sigs->ptr(), legendre_order+1, num_groups, num_groups, 3);
...
void Kernel_3d_ZDG::sweep(Subdomain *sdom) {
  ...
  View3d<double, LAYOUT_KIJ> const rhs(sdom->rhs->ptr(), num_directions, num_groups, num_zones);
  View3d<double, LAYOUT_KIJ> psi(sdom->psi->ptr(), num_directions, num_groups, num_zones);
  ...
  View3d<double, LAYOUT_KIJ> psi_lf(sdom->plane_data[0]->ptr(), num_directions, num_groups, num_z_i);
  View3d<double, LAYOUT_KIJ> psi_fr(sdom->plane_data[1]->ptr(), num_directions, num_groups, num_z_j);
  View3d<double, LAYOUT_KIJ> psi_bo(sdom->plane_data[2]->ptr(), num_directions, num_groups, num_z_k);
