...
void zgd_nmd_SweepDD_1d(zgd_nmd_Param &p){
  ...
  zgd_TVec psi_bo_v(p.num_groups, p.num_directions+1, p.num_zones);
  double ***psi_bo = psi_bo_v.data;

  zgd_TVec psi_l_v(p.num_groups, p.num_directions, p.num_zones+1);
  zgd_TVec psi_r_v(p.num_groups, p.num_directions, p.num_zones+1);
  ...
  for(int group = 0; group < p.num_groups; ++group){
    double val = p.psi.data[0][group][0];
    psi_r1[group] = val;
  ...
  for(int i=num_zones-1; i>=0; i--){
    ...
    for(int group = 0; group < p.num_groups; ++group){
      ...
      psi_r1[group] += psi_internal_in_g[i];   /* Add internal source if any
                                                 */
      double dxip1_RHS_d0_g_z = dxip1*RHS_z[group][0];
      ...
      psi_bo_z[group][0] = psi_bo_d0_z_g;
    }
  }
#ifdef _OPENMP
#pragma omp parallel for
  ...
  for(int d=0; d<Lm; d++){
    for(int group = 0; group < p.num_groups; ++group){
      ...
      psi_r[num_zones][group][d] = psi_z0_d0_init[group];
    }
  }
#ifdef _OPENMP
#pragma omp parallel for
...
#pragma omp parallel for
    ...
    for(int group = 0; group < p.num_groups; ++group){
      ...
      double * __restrict__ psi_r_z_g = psi_r_z[group];
      double * __restrict__ psi_r_zp1_g = psi_r_zp1[group];

      double sigt_z_g = sigt_z[group];
      ...
      for(int d=0; d<Lm; d++){
        ...
        psi_r_zp1_g[d] += psi_internal_in_g[i]; /* Add internal source if any
                                                  */

        double psi_z_g_d = 2.0 *
                           ( A1_tmp*psi_r_zp1_g[d] + A2_tmp*
                             psi_bo_z_g[d]
                             + Vi*
                             RHS_z_g[d] ) /
                           ( Vi*sigt_z_g + 2.0*mu*Ai + A3_tmp );
        psi_r_z_g[d] = psi_z_g_d - psi_r_zp1_g[d];
        psi_bo_z_g[d+1] = psi_z_g_d - psi_bo_z_g[d];
        psi_z_g[d] = 0.5*psi_z_g_d;
      }
    }
  }
...
#endif
  for(int group = 0; group < p.num_groups; ++group){
    double * __restrict__ psi_r_z0_g = psi_r_z0[group];
    double * __restrict__ psi_z0_g = psi_z0[group];
    double * __restrict__ psi_r_z1_g = psi_r_z1[group];
    double * __restrict__ psi_bo_z0_g = psi_bo_z0[group];
    double * __restrict__ RHS_z0_g = RHS_z0[group];
    ...
    for(int d=0; d<Lm; d++){
      ...
      double psi_d_z0_g =
        ( mu*(A1*psi_r_z1_g[d] - A0*psi_r_z0_g[d]) +
          (A1-A0)/(w)*(aup+adn)*psi_bo_z0_g[d]
          + V0*RHS_z0_g[d] ) / ( V0*sigt_z0[group] + 2.0*(A1-A0)*aup/w );

      psi_bo_z0_g[d+1] = 2.0*psi_d_z0_g - psi_bo_z0_g[d];
      psi_z0_g[d] = psi_d_z0_g;
    }
  }
...
#pragma omp parallel for
    ...
    for(int group = 0; group < p.num_groups; ++group){
      ...
      double * __restrict__ RHS_z_g = RHS_z[group];
      double * __restrict__ psi_bo_z_g = psi_bo_z[group];
      double * __restrict__ psi_l_z_g = psi_l_z[group];
      double * __restrict__ psi_l_zp1_g = psi_l_zp1[group];
      ...
      for(int d=Lm; d<num_directions; d++){
        ...
        psi_l_z_g[d] += psi_internal_out_g[i]; /* Add internal source if any
                                                  */

        double psi_z_g_d = 2.0 *
                           ( A1_tmp*psi_l_z_g[d] + A2_tmp*
                             psi_bo_z_g[d]
                             + Vi*
                             RHS_z_g[d] ) /
                           ( Vi*sigt_z_g + 2.0*mu*Aip1 + A3_tmp );
        psi_l_zp1_g[d] = psi_z_g_d - psi_l_z_g[d];
        psi_bo_z_g[d+1] = psi_z_g_d - psi_bo_z_g[d];
        psi_z_g[d] = 0.5*psi_z_g_d;
      }
    }
  ...
  for(int group = 0; group < p.num_groups; ++group){
    ...
    for(int d=Lm; d<num_directions; d++){
      psi_out[group][d] = psi_l[num_zones][group][d];
    }
  }
}
...
void zgd_nmd_Param::SweepDD(void) {

  if(geometry_type == 1){
    zgd_nmd_SweepDD_1d(*this);
  }
  else if(geometry_type == 2){
    //zgd_nmd_SweepDD_2d(*this);
  }
  else {
    zgd_nmd_SweepDD_3d(*this);
  }
}
