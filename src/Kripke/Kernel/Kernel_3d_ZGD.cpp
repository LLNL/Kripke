...
void Kernel_3d_ZGD::sweep(Subdomain *sdom) {
  ...
  View3d<double, PERM_KJI> const rhs(sdom->rhs->ptr(), num_directions, num_groups, num_zones);
  View3d<double, PERM_KJI> psi(sdom->psi->ptr(), num_directions, num_groups, num_zones);
  View2d<double, PERM_JI>  const sigt(sdom->sigt->ptr(), num_groups, num_zones);

  int num_z_i = local_jmax * local_kmax;
  int num_z_j = local_imax * local_kmax;
  int num_z_k = local_imax * local_jmax;  
  
  View3d<double, PERM_KJI> psi_lf(sdom->plane_data[0]->ptr(), num_directions, num_groups, num_z_i);
  View3d<double, PERM_KJI> psi_fr(sdom->plane_data[1]->ptr(), num_directions, num_groups, num_z_j);
  View3d<double, PERM_KJI> psi_bo(sdom->plane_data[2]->ptr(), num_directions, num_groups, num_z_k);
