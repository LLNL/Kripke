      ...
      for (int g = 0; g < num_local_groups; ++g) {
        for(int nm = 0;nm < num_moments;++nm){
          for (int d = 0; d < num_local_directions; d++){
            phi[z*num_gnm + (group0+g)*num_moments + nm] += 
              ell[nm*num_local_directions + d] * 
              psi[z*num_locgd + g*num_local_directions + d];
          }
        }
      }
    }
...
#pragma omp parallel for
    ...
    for(int z = 0;z < num_zones; ++ z){    
      for (int g = 0; g < num_local_groups; ++g) {
        for (int d = 0; d < num_local_directions; d++) {
          for(int nm = 0;nm < num_moments; ++nm){
            rhs[z*num_locgd + g*num_local_directions + d] += 
              ell_plus[d*num_moments + nm] * 
              phi_out[z*num_gnm + (group0+g)*num_moments + nm];
          }          
        }        
      }     
    }
        ...
        for(int g = 0;g < num_groups;++ g){      
          for(int gp = 0;gp < num_groups;++ gp){
            for(int nm = 0;nm < num_moments;++ nm){
              ...
              phi_out[zone*num_nmg + gp*num_moments + nm] += 
                sigs[material*num_coeff*num_groups*num_groups + g*num_groups*num_coeff + gp*num_coeff + n] * 
                phi[zone*num_nmg + g*num_moments + nm] * 
                fraction;
            }
          }        
        }
...
#pragma omp parallel for
    ...
    for(int zone = 0;zone < num_zones;++ zone){
      ...
      for(int mix = mix_start;mix < mix_stop;++ mix){
        ...
        if(material == 0){        
          for(int g = 0;g < num_groups;++ g){
            phi_out[zone*num_moments*num_groups + g*num_moments] += 1.0 * fraction;
          }
        }
      }
    }
...
#pragma omp parallel for
        ...
        for (int g = 0; g < num_groups; ++g) {
          for (int d = 0; d < num_directions; ++d) {
            ...
            int const lf_idx = I_PLANE_INDEX(j, k)*num_gd + g*num_directions + d;
            int const fr_idx = J_PLANE_INDEX(i, k)*num_gd + g*num_directions + d;
            int const bo_idx = K_PLANE_INDEX(i, j)*num_gd + g*num_directions + d;
            
            // Calculate new zonal flux 
            double const psi_z_g_d = (
                rhs[z*num_gd + g*num_directions + d]
                + psi_lf[lf_idx] * xcos_dxi
                + psi_fr[fr_idx] * ycos_dyj
                + psi_bo[bo_idx] * zcos_dzk)
                / (xcos_dxi + ycos_dyj + zcos_dzk + sigt[z*num_groups + g]);

            psi[z*num_gd + g*num_directions + d] = psi_z_g_d;

            // Apply diamond-difference relationships 
            psi_lf[lf_idx] = 2.0 * psi_z_g_d - psi_lf[lf_idx];
            psi_fr[fr_idx] = 2.0 * psi_z_g_d - psi_fr[fr_idx];
            psi_bo[bo_idx] = 2.0 * psi_z_g_d - psi_bo[bo_idx];
          }
        }
      }
    }
