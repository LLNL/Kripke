...
void Kernel_3d_ZGD::sweep(Grid_Data *grid_data, Group_Dir_Set *gd_set,
  ...
  for(int k=0; k<local_kmax; k++){
    for(int j=0; j<local_jmax; j++){
      double ** psi_lf_z =
        psi_lf.data[Left_INDEX(istartz+il, j, k)];
      double ** i_plane_z =
  ...
  for(int k=0; k<local_kmax; k++){
    for(int i=0; i<local_imax; i++){
      double ** psi_fr_z =
        psi_fr.data[Front_INDEX(i, jstartz+jf, k)];
      double ** j_plane_z =
  ...
  for(int j=0; j<local_jmax; j++){
    for(int i=0; i<local_imax; i++){
      double ** psi_bo_z =
        psi_bo.data[Bottom_INDEX(i, j, kstartz+ kb)];
      double ** k_plane_z =
  ...
  for(int k=0; k<local_kmax; k++){
    for(int j=0; j<local_jmax; j++){
      double ** psi_lf_z =
        psi_lf.data[Left_INDEX(istopz+ir, j, k)];
      double ** i_plane_z =
        i_plane[I_PLANE_INDEX(j, k)];
  ...
  for(int k=0; k<local_kmax; k++){
    for(int i=0; i<local_imax; i++){
      double ** psi_fr_z =
        psi_fr.data[Front_INDEX(i, jstopz+jb, k)];
      double ** j_plane_z =
        j_plane[J_PLANE_INDEX(i, k)];
  ...
  for(int j=0; j<local_jmax; j++){
    for(int i=0; i<local_imax; i++){
      double ** psi_bo_z =
        psi_bo.data[Bottom_INDEX(i, j, kstopz+kt)];
      double ** k_plane_z =
        k_plane[K_PLANE_INDEX(i, j)];
