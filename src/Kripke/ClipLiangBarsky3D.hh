...
namespace KKC_CLIP_3D {
  ...
  /* We make a slight modification in an attempt to make the clipping more robust to degenerate intersections and lines parallel to edges.
     ...
     or an implementation of Simulation of Simplicity.
  */

  // this is just a struct to help clean things up a bit in the code, there is an interface that avoids calling codes needing to know about this
  template<typename T>
  struct Vector3D {
    T x[3];
    inline Vector3D(const T x0=0, const T x1=0, const T x2=0)
    {
      x[0] = x0;
      x[1] = x1;
      x[2] = x2;
		 ...
		 T &t_min, T &t_max) // input and output: the parameter space endpoints of the clipped line, if it is clipped
  {
    ...
    if ( denom > std::abs(line_scale) ) //this kind of test is DANGEROUS, except because we perturb the points before working with them it should be "ok".  A serious algorithm would use robust predicates or SOS instead of this test.
      { // entering itersection
	t = norm_dot_dir/denom;
	if ( (t-t_max)>std::numeric_limits<T>::epsilon() ) // note that t\in[0,1] is an O(1) number, so this comparison is appropriate, however we should really use a robust predicate 
	  accept=false;
	else if ( (t-t_min)>std::numeric_limits<T>::epsilon() )
	  t_min = t; // found a new t_min on the line segment
      }
    else if ( denom < -std::abs(line_scale) ) //this kind of test is DANGEROUS, except because we perturb the points before working with them it should be "ok".  A serious algorithm would use robust predicates or SOS instead of this test.
      { // leaving intersection
	t = norm_dot_dir/denom;
	if ( (t_min-t)>std::numeric_limits<T>::epsilon() ) // note that t\in[0,1] is an O(1) number, so this comparison is appropriate, however we should really use a robust predicate 
	  accept=false;
	else if ( (t_max-t)>std::numeric_limits<T>::epsilon() )
	  t_max = t; // found a new t_max on the line segment
      }
    else if ( norm_dot_dir > dir_scale ) // you know, should this ever happen with perturbed lines?  If we used robust predicates instead it would occur for sure.
      { 
	accept = false;
      }
    return accept;
