...
#       If your compiler needs a flag to specify the version of MPI to use,
#       then set MPI_VERSION_FLAG with the needed flag.  You will still need
#       to include this flag in your normal CMAKE_<lang>_FLAGS variable.
...
set(_MPI_EXEC_NAMES                        srun mpiexec mpirun lamexec )
...
function (interrogate_mpi_compiler lang try_libs)
  ...
  if (interrogate AND ((NOT MPI_${lang}_INCLUDE_PATH) OR (NOT MPI_${lang}_LIBRARIES)))
    if (MPI_${lang}_COMPILER)
      ...
      execute_process(
        COMMAND ${MPI_${lang}_COMPILER} ${MPI_VERSION_FLAG} -showme:compile
      ...
      if (MPI_COMPILER_RETURN EQUAL 0)
        ...
        execute_process(
          COMMAND ${MPI_${lang}_COMPILER} ${MPI_VERSION_FLAG} -showme:link
        ...
        if (MPI_COMPILER_RETURN EQUAL 0)
          ...
          execute_process(
            COMMAND ${MPI_${lang}_COMPILER} ${MPI_VERSION_FLAG} -showme:incdirs
            OUTPUT_VARIABLE  MPI_INCDIRS OUTPUT_STRIP_TRAILING_WHITESPACE
            ERROR_VARIABLE   MPI_INCDIRS ERROR_STRIP_TRAILING_WHITESPACE)

          execute_process(
            COMMAND ${MPI_${lang}_COMPILER} ${MPI_VERSION_FLAG} -showme:libdirs
      ...
      if (NOT MPI_COMPILER_RETURN EQUAL 0)
        execute_process(
          COMMAND ${MPI_${lang}_COMPILER} ${MPI_VERSION_FLAG} -showme
          OUTPUT_VARIABLE  MPI_COMPILE_CMDLINE OUTPUT_STRIP_TRAILING_WHITESPACE
          ERROR_VARIABLE   MPI_COMPILE_CMDLINE ERROR_STRIP_TRAILING_WHITESPACE
          RESULT_VARIABLE  MPI_COMPILER_RETURN)
      endif()
      ...
      if (NOT MPI_COMPILER_RETURN EQUAL 0)
        execute_process(
          COMMAND ${MPI_${lang}_COMPILER} ${MPI_VERSION_FLAG} -compile-info
          OUTPUT_VARIABLE  MPI_COMPILE_CMDLINE OUTPUT_STRIP_TRAILING_WHITESPACE
          ERROR_VARIABLE   MPI_COMPILE_CMDLINE ERROR_STRIP_TRAILING_WHITESPACE
          RESULT_VARIABLE  MPI_COMPILER_RETURN)
        ...
        if (MPI_COMPILER_RETURN EQUAL 0)
          execute_process(
            COMMAND ${MPI_${lang}_COMPILER} ${MPI_VERSION_FLAG} -link-info
            OUTPUT_VARIABLE  MPI_LINK_CMDLINE OUTPUT_STRIP_TRAILING_WHITESPACE
            ERROR_VARIABLE   MPI_LINK_CMDLINE ERROR_STRIP_TRAILING_WHITESPACE
            RESULT_VARIABLE  MPI_COMPILER_RETURN)
        endif()
      ...
      if (NOT MPI_COMPILER_RETURN EQUAL 0)
        execute_process(
          COMMAND ${MPI_${lang}_COMPILER} ${MPI_VERSION_FLAG} -show
          OUTPUT_VARIABLE  MPI_COMPILE_CMDLINE OUTPUT_STRIP_TRAILING_WHITESPACE
          ERROR_VARIABLE   MPI_COMPILE_CMDLINE ERROR_STRIP_TRAILING_WHITESPACE
          RESULT_VARIABLE  MPI_COMPILER_RETURN)
      endif()
...
# work with it.  Works on machines like Cray XE6, where the modules environment changes 
# what MPI version cc, CC, and ftn use.
function(try_regular_compiler lang success)
  ...
  if (compiler_has_mpi)
    ...
    interrogate_mpi_compiler(${lang} FALSE) 
...
foreach (lang ${enabled_langs})
  if (CMAKE_${lang}_COMPILER_WORKS)

    # First try running the regular compiler and
